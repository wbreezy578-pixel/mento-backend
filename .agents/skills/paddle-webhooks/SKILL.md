...
Concretely: don't run `paddle.transactions.list()` inline inside a webhook handler. Don't send an email inline. Don't call other third-parties from inside the route handler. Verify, queue, ack.
