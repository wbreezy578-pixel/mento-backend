...
export async function POST(req: Request) {
  ...
  if (!token) return NextResponse.json({ error: 'Confirmation link is invalid.' }, { status: 400 });
  ...
  if (!ipLimit.ok || !tokenLimit.ok) return NextResponse.json({ error: 'Too many confirmation attempts. Please try again later.' }, { status: 429 });
  ...
  if (result.status === 'invalid') return NextResponse.json({ error: 'Confirmation link is invalid or expired.' }, { status: 401 });
  if (result.status === 'conflict') return NextResponse.json({ error: 'Email is already in use.' }, { status: 409 });
