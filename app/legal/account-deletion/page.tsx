import LegalPage from '../LegalPage';

export const metadata = {
  title: 'Delete Your Mento Account',
  description: 'Instructions for permanently deleting your Mento account and associated learning data.',
  alternates: { canonical: '/legal/account-deletion' },
};

export default function AccountDeletionPage() {
  return <LegalPage title="Delete your Mento account" updated="September 14, 2026">
    <h2>Delete from the app</h2>
    <ol>
      <li>Open Mento and sign in.</li>
      <li>Open Settings and choose Delete Account.</li>
      <li>Complete the security confirmation and type “delete my account”.</li>
    </ol>
    <p>We process deletion requests promptly. Account credentials, sessions, conversations, settings, and learning data are deleted. If an identity or service provider is temporarily unavailable, we keep the request securely and retry it. Completed payment and fraud-prevention records may be retained in anonymized form where legally required.</p>
    <h2>If you cannot access the app</h2>
    <p>Email <a href="mailto:mentosupport@gmail.com?subject=Mento%20account%20deletion%20request">mentosupport@gmail.com</a> from the address on your Mento account. We will verify account ownership before deletion.</p>
    <h2>Subscriptions</h2>
    <p>Deleting a Mento account does not itself cancel a Google Play subscription. Cancel any active subscription in Google Play to prevent future renewal, then confirm its status there. Google Play controls store purchase records and refund decisions.</p>
  </LegalPage>;
}
